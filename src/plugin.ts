...
import { TASK_NODE_SERVER_READY, TASK_TEST_SETUP_TEST_ENVIRONMENT } from 'hardhat/builtin-tasks/task-names';
...
task(TASK_NODE_SERVER_READY).setAction((_, env, runSuper) =>
  runSuper().then(() => Promise.all(
    Object.entries(env.config.predeploy).map(([ address, { name, abi, bytecode }]) =>
