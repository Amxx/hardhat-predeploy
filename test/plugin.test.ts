...
it("bytecode is deployed", async () => {
  for (const [address, { bytecode }] of Object.entries(
    hre.config.predeploy,
...
it("disabled predeploys are not deployed", async () => {
  for (const [address] of Object.entries(hre.config.predeploy).filter(
    ([, details]) => !details,
  )) {
...
it("hre.predeploy is populated", () => {
  for (const [address, { name }] of Object.entries(hre.config.predeploy).filter(
    ([, details]) => details,
  )) {
    const contract = name
      .split(".")
      .reduce((container, key) => container && container[key], hre.predeploy);
